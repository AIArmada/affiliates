...
| `deactivated_at` | timestampTz nullable | When deactivated | OK |
| `paused_at` | timestampTz nullable | When paused | OK |
...
**Status**: Timestamps are now recorded on transition to Disabled (`deactivated_at`) and Paused (`paused_at`).
...
| `rejected_at` | timestampTz nullable | When rejected | OK |
| `paid_at` | timestampTz nullable | When paid | OK |

**Status**: `rejected_at` and `paid_at` timestamps are now set on state transitions.
...
| `failed_at` | timestampTz nullable | When failed | OK |
| `cancelled_at` | timestampTz nullable | When cancelled | OK |
...
| `rejected_at` | timestampTz nullable | When rejected | OK |
| `suspended_at` | timestampTz nullable | When suspended | OK |
...
**Status**: `is_active` replaced by `deactivated_at` timestamp. `isEnabled()` accessor available.
...
| `dismissed_at` | timestampTz nullable | When dismissed | OK |
| `confirmed_at` | timestampTz nullable | When confirmed | OK |
